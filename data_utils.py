...
hex_revesred_dict = {v: [int(c) for c in k] for k, v in hex_dict.items()}
...
def xor_lists(l1:list, l2:list) -> list:
    assert len(l1) == len(l2), "Two lists must have the same size"
    l = []
    for d1, d2 in zip(l1, l2):
        l.append(d1^d2)
    return l

def joinNumbers(l:list) -> str:
    s = ''
    for n in l:
        s += str(n)
    return s
